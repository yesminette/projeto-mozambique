...
//	Configuration
//
var fileLoadingImage = contextPath+"/resources/images/loading.gif";				// Loading image
var fileBottomNavCloseImage = contextPath+"/resources/images/closelabel.gif";		// Close image
var fallbackOverlayImage = contextPath+"/resources/images/overlay.png";			// Fallback overlay used with browsers that have opacity problems
...
Lightbox.prototype = {
	...
	start: function(elementLink) {	
		...
		// Image or SWF is not part of a set...
			...
			// Loop through anchors, find other /resources/images/SWF's in the set, and add them to elementArray
			for (var i=0; i<anchors.length; i++){
				var anchor = $(anchors[i]);
		...
		if (images){
			...
			for (var i = 0; i < images.length; i++){
				if ((images[i].src.indexOf(".png")) != -1){
					...
					images[i].parentNode.style.filter = "progid:dximagetransform.microsoft.alphaimageloader(src=contextPath+'/resources/images/"+ srcname +".png',sizingmethod='crop');";
				}
			}
		}
	}
